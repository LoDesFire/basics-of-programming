...
bool FileStream::loadFromFile(List *&list) {
    ...
    while (!stream->atEnd() && isValid) {
        ...
        int count = 0;
        for (; count < 4 && !stream->atEnd(); count++) {
            arrStr[count] = stream->readLine();
        }
        if (count != 4) return false;

        Employee *tmp = Employee::parse(arrStr);
...
bool FileStream::saveTheFile(List *&list) {
    ...
    if (list)
        for (unsigned long long i = 0; i < list->get_size(); i++) {
            (*stream) << *((list->operator [](i))->data->toString());
        }
...
bool FileStream::saveToFile(List *&list) {
    FileStream *fs = new FileStream(QFileDialog::getSaveFileName(nullptr, "Open File", "/Users/lllymuk/Desktop/OAiP/OAiP 2 sem/LW2_T1_1/"));
