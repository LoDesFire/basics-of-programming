...
protected:
    ...
    bool searchNode(Node<T>*, int, void (*)(Node<T>*));
...
public:
    ...
    bool searchByKey(int key, void (*)(Node<T>*));
...
template<typename T>
bool Tree<T>::searchNode(Node<T> *node, int key, void (*printOneNode)(Node<T>*)) {
    if (!node) {
    ...
    if (key < node->data) {
        return searchNode(node->left, key, printOneNode);
    }
    else if (key > node->data) {
        return searchNode(node->right, key, printOneNode);
    }
    else {
        printOneNode(node);
        return 1;
...
template<typename T>
bool Tree<T>::searchByKey(int key, void (*printOneNode)(Node<T>*)){
    if (!root) {
        return 0;
    }

    return searchNode(root, key, printOneNode);
}
